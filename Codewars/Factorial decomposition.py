...
#     print(result)
    for j in range(n, 1, -1):
        for ind in range(2, j):
            if j % ind == 0:
                result[ind]    = result[ind]+result[j]
                result[j//ind] = result[j//ind]+result[j]
                result[j]      = 0
#                 print(j, ind, j//ind, result)
                break
#     print(result)
    result_str = ''
    for facto in range(2, n+1):
        if result[facto]>0:
            if result[facto]>1:
                result_str += ' * ' + str(facto) + '^' + str(result[facto])
            else:
                result_str += ' * ' + str(facto)
    return result_str[3:]
